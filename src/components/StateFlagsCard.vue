  ...
  <div class="bg-white rounded-lg shadow-sm p-4 mb-4">
    ...
    <div v-if="!verboseMode" class="space-y-4">
      <div v-for="group in groupedStateFlags" :key="group.id" class="space-y-2">
        <template v-if="group.flags.some(f => f?.active)">
          <h4 class="text-xs font-semibold text-gray-600 uppercase tracking-wider">{{ group.name }}</h4>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            <div v-for="flag in group.flags.filter(f => f?.active)" :key="flag?.name" class="flex items-center space-x-3 text-sm p-2 bg-green-50 rounded-md border border-green-200">
    ...
    <div v-else class="space-y-4">
      <div v-for="group in groupedStateFlags" :key="group.id" class="space-y-2">
        <h4 class="text-xs font-semibold text-gray-600 uppercase tracking-wider">{{ group.name }}</h4>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          <div v-for="flag in group.flags" :key="flag?.name" class="flex items-center space-x-3 text-sm p-2 rounded-md border" :class="flag?.active ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'">
